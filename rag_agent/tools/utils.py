...
def check_corpus_exists(corpus_name: str, tool_context: Optional[ToolContext] = None) -> bool:
    ...
    """
    # Check state first if tool_context is provided and has a state mapping
    if tool_context is not None and getattr(tool_context, "state", None) is not None:
        try:
            ...
            # If state access fails for any reason, ignore caching and continue
            pass
            ...
            if (
                ...
                if tool_context is not None and getattr(tool_context, "state", None) is not None:
                    try:
                        ...
                        # If state update fails, ignore it
                        pass
                return True
...
def set_current_corpus(corpus_name: str, tool_context: Optional[ToolContext] = None) -> bool:
    """
